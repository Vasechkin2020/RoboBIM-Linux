...
Eigen::Vector2d fitCircleFixedRadius(const std::vector<Eigen::Vector2d>& points, double target_radius) {
    ...
    for (int iter = 0; iter < 2; ++iter) {
        ...
        if (sum_w > 1e-6) center += grad / sum_w;
    }
    return center;
}
...
private:
    ...
    void processScans(const ros::TimerEvent& event) {
        ...
        for (const auto& scan : scans_) {
            for (size_t i = 0; i < scan.ranges.size(); ++i) {
                ...
                if (r > 15.0) continue; // фильтр по радиусу
        ...
        if (all_points.size() > 100) {
            ...
            for (size_t i = 0; i < all_points.size(); ++i) {
                ...
                for (size_t j = 0; j < all_points.size(); ++j) {
                    if (i == j) continue;
                    if (dist2D(all_points[i], all_points[j]) < 0.3) neighbors++;
                }
                if (neighbors >= 3) filtered_points.push_back(all_points[i]);
            }
        } else {
        ...
        for (const auto& seg : candidates_segments) {
            if (seg.size() < 5) continue; // слишком мало точек
            ...
            int method_id = 0;
            if (seg_idx < 10) method_id = 0; // метод 1 — первые
            else if (seg_idx < 30) method_id = 1; // метод 2
            else method_id = 2; // метод 3
            w *= method_weights[method_id];

            if (w > 0.01) { // порог значимости
        ...
        for (int i = 0; i < 4; ++i) P[i] = fused[i].center;
    ...
    void detectByRangeDiscontinuities(const sensor_msgs::LaserScan& scan, std::vector<std::vector<Eigen::Vector2d>>& segments) {
        ...
        for (size_t i = 0; i < ranges.size() - 1; ++i) {
            if (std::isnan(ranges[i]) || std::isnan(ranges[i+1])) continue;
            if (ranges[i] < scan.range_min || ranges[i] > scan.range_max) continue;
            if (ranges[i+1] < scan.range_min || ranges[i+1] > scan.range_max) continue;
        ...
        size_t start = 0;
        for (size_t gap : gaps) {
            if (gap - start < 3) { // минимум 3 точки
                start = gap + 1;
                continue;
            }
    ...
    void detectByClustering(const std::vector<Eigen::Vector2d>& points, std::vector<std::vector<Eigen::Vector2d>>& segments) {
        ...
        for (size_t i = 0; i < points.size(); ++i) {
            ...
            if (cluster.size() >= min_pts) {
                std::vector<Eigen::Vector2d> seg;
                for (size_t idx : cluster) seg.push_back(points[idx]);
                // Проверка диаметра
                double max_d = 0.0;
    ...
    void detectByLocalMinima(const sensor_msgs::LaserScan& scan, std::vector<std::vector<Eigen::Vector2d>>& segments) {
        ...
        for (size_t i = 5; i < smoothed.size() - 5; ++i) {
            ...
            int end = i + half_win;
            if (start < 0 || end >= (int)smoothed.size()) continue;
            ...
            if (count == 0) continue;
            mse /= count;
            if (mse > 0.0016) continue; // (40 mm)^2

            // Собираем точки
            std::vector<Eigen::Vector2d> seg;
            for (int j = start; j <= end; ++j) {
                if (std::isnan(ranges[j])) continue;
