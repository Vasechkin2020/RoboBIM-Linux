...
#define LOG(fmt, ...) do { logi.log((fmt), ##__VA_ARGS__); } while(0)
...
public:
    ...
    void addScan(const std::vector<double>& ranges) {
        ...
        for (int i = 0; i < angular_steps_; ++i) {
            ...
            if (buffer_[i].size() > max_scans_) buffer_[i].pop_front();
        }
        collected_++;
    }
    ...
    void reset() {
        for (int i = 0; i < angular_steps_; ++i) buffer_[i].clear();
        collected_ = 0;
    }
    ...
    std::vector<double> getMedianScan(double min_range=DEFAULT_MIN_RANGE, double max_range=DEFAULT_MAX_RANGE) const {
        ...
        for (int i = 0; i < angular_steps_; ++i) {
            ...
            for (double v : tmp) {
                if (!std::isnan(v) && v >= min_range && v <= max_range) valid.push_back(v);
            }
            if (valid.empty()) {
                out[i] = std::numeric_limits<double>::quiet_NaN();
                continue;
...
public:
    ...
    std::vector<Cluster> cluster(const std::vector<Point2D>& pts, const std::vector<int>& idxs_in_scan) {
        ...
        enum PtState { UNVISITED=0, VISITED, NOISE, CLUSTERED };
        ...
        while (!q.empty()) {
            ...
            if (state[v] == UNVISITED) {
                ...
                if ((int)neigh2.size() >= min_pts_) {
                    for (int t : neigh2) q.push_back(t);
                }
            }
...
public:
    ...
    std::vector<Cluster> detect(const std::vector<double>& ranges) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            for (int j = a; j <= b; ++j) {
                double r = ranges[j];
                if (!std::isnan(r)) window.push_back(r);
            }
            if (!window.empty()) {
        ...
        for (int i = 0; i < N; ++i) {
            if (std::isnan(ranges[i]) || std::isnan(bg[i])) continue;
            if (bg[i] - ranges[i] > shadow_thresh_) mark[i] = 1;
        }
        // соберём последовательные помеченные
        int i = 0;
        while (i < N) {
            if (mark[i] == 0) { ++i; continue; }
            int a = i;
            while (i < N && mark[i] == 1) ++i;
            int b = i - 1;
            Cluster cl;
            cl.method = "shadow";
            for (int k = a; k <= b; ++k) cl.idxs.push_back(k);
            cl.N = cl.idxs.size();
...
public:
    ...
    static std::tuple<Point2D,double,double> fitCircle(const std::vector<Point2D>& pts) {
        ...
        double rmse = 1e9;
        int n = pts.size();
        if (n < 3) return {center, radius, rmse};

        // центровать данные
        double meanx = 0.0, meany = 0.0;
        for (auto &p: pts) { meanx += p.x; meany += p.y; }
        meanx /= n; meany /= n;
        ...
        int minIdx = 0;
        for (int i=1;i<3;++i) if (evals[i] < evals[minIdx]) minIdx = i;
...
public:
    ...
    std::pair<std::vector<Point2D>, double> matchFour(const std::vector<Point2D>& found4) {
        ...
        do {
            ...
            for (int i=0;i<4 && i < (int)found4.size(); ++i) assigned[i] = found4[perm[i]];
    ...
    double computePairwiseRMS(const std::vector<Point2D>& asg, const std::vector<Point2D>& ref) {
        ...
        for (auto &pr: pairs) {
            ...
            if (i >= (int)asg.size() || j >= (int)asg.size()) { sumsq += 1.0; cnt++; continue; }
...
private:
    ...
    double getTrueDistance(int i, int j) const {
        ...
        else if (i==0 && j==3) idx=2;
        else if (i==1 && j==2) idx=3;
        else if (i==1 && j==3) idx=4;
        else if (i==2 && j==3) idx=5;
        if (idx==-1) return 0.0;
        return d_true_[idx];
    }
    ...
    std::pair<std::pair<Point2D,Point2D>, bool> circleIntersection(const Point2D& p0, double r0, const Point2D& p1, double r1) {
        ...
        if (d < 1e-9) return {{ {0,0},{0,0} }, false}; // совпадают
        if (d > r0 + r1 + 1e-6) return {{ {0,0},{0,0} }, false}; // нет пересечений
        if (d < fabs(r0 - r1) - 1e-6) return {{ {0,0},{0,0} }, false}; // одна внутри другой
...
public:
    ...
    static std::pair<Point2D,double> weightedAverage(const std::vector<std::pair<Point2D,double>>& samples) {
        Point2D out{0.0,0.0};
        double sumw = 0.0;
        if (samples.empty()) return {out, 1e9};
        for (auto &s : samples) {
...
public:
    ...
    static Similarity estimate(const std::vector<Point2D>& P, const std::vector<Point2D>& Q, bool estimate_scale=true) {
        size_t n = P.size();
        Similarity out;
        out.scale = 1.0; out.rot = 0.0; out.trans = {0.0,0.0};
        if (n == 0) return out;
        // centroids
        Point2D muP{0.0,0.0}, muQ{0.0,0.0};
        for (size_t i=0;i<n;++i) { muP.x += P[i].x; muP.y += P[i].y; muQ.x += Q[i].x; muQ.y += Q[i].y; }
        ...
        if (estimate_scale) {
            ...
            traceSR = tmp.trace();
            if (fabs(varP) < 1e-12) scale = 1.0;
            else scale = traceSR / varP;
        }
...
public:
    ...
    : nh_(nh)
    {
        ...
        for (int i=0;i<6;++i) dtrue[i] = dist_pillar[i];
...
private:
    ...
    void scanCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
        ...
        for (int i=0;i<N;++i) ranges[i] = msg->ranges[i];
        // Добавим в агрегатор
        ...
        for (auto &cl : clusters) {
            ...
            for (int idx : cl.idxs) {
                if (idx >= 0 && idx < (int)pts_full.size()) {
                    auto p = pts_full[idx];
                    if (!std::isnan(p.x) && !std::isnan(p.y)) cl.pts.push_back(p);
                }
            }
            cl.N = cl.pts.size();
        }
    ...
    void evaluateClusters(std::vector<Cluster>& clusters) {
        for (auto &cl : clusters) {
            ...
            cl.center = center; cl.radius = radius; cl.rmse = rmse;
    ...
    std::vector<std::pair<Point2D,double>> produceNamedByMethod(const std::string& method, const std::vector<Cluster>& clusters) {
        ...
        for (auto &c : clusters) if (c.method == method) sel.push_back(c);
        if (sel.empty()) {
            LOG("[produceNamedByMethod] method=%s: нет кластеров\n", method.c_str());
            return res;
        }
        // Сортировка по confidence (убывание)
        std::sort(sel.begin(), sel.end(), [](const Cluster&a, const Cluster&b){ return a.confidence > b.confidence; });
        ...
        if (centers.size() >= 4) {
            ...
            for (int i=0;i<4;++i) first4[i] = centers[i];
