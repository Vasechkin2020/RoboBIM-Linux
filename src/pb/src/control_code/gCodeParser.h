...
private:
    ...
    const float wheel_base_m_ = 0.38f;        // Расстояние между колёсами в метрах (база робота).
    ...
    void executeG0(const GCodeCommand &cmd) // Обработка команды G0 (движение по заданным скоростям колес F_L, F_R в течение времени T).
    {
        ...
        float omega = (cmd.f_r - cmd.f_l) / wheel_base_m_; // Угловая скорость робота (рад/с).
    ...
    void executeG1(const GCodeCommand &cmd) // Обработка команды G1 (поворот на угол A/O или к точке X/Y).
    {
        ...
        float omega_rad_per_sec = (2.0f * wheel_speed) / wheel_base_m_; // Угловая скорость (рад/с).
    ...
    void run() // Основная функция для парсинга файла и симуляции выполнения команд.
    {
        ...
        if (!commandArray.empty())                                                         // Если массив команд не пуст.
        {
            for (size_t i = 0; i < commandArray.size(); ++i) // Цикл для вывода информации о каждой команде.
            {
                ...
                printf("Cmd[%zu]: mode=%d, dur=%.1f, vL=%.3f, vR=%.3f, "             // Используем printf (по вашему предпочтению) для вывода информации о команде.
                       "delta_angle=%.3f, vel_angle=%.3f, len=%.3f, vel_len=%.3f | " // Продолжение вывода параметров движения.
