...
private:
    ...
    void executeG1(const GCodeCommand &cmd) // Обработка команды G1 (поворот на угол A/O или к точке X/Y).
    {
        ...
        command.control_source = 1;     // --- ВЫБОР РЕЖИМА УПРАВЛЕНИЯ --- // Для поворотов используем MODEL (Гироскоп). // Он плавный и не дает перекрутить лишнего.
    ...
    void executeG2(const GCodeCommand &cmd) // Обработка команды G2 (линейное движение на L или к X/Y).
    {
        ...
        if (cmd.has_l) 
        {
            command.control_source = 0;  // Если задана Длина (L) - это локальный маневр (отъезд пера).  Используем ODOM (чистые энкодеры), чтобы шум лидара/гироскопа не сдвинул робота.
        }
        else 
        {
            command.control_source = 2;  // Если заданы Координаты (X/Y) - это глобальный проезд.  Используем EST (Лидар), чтобы корректировать траекторию.
        }
