...
// global_mode: глобальная настройка из конфига (0=Odom, 1=Model, 2=Est)
// cmd_mode: требование текущей команды (0=Force Odom, 1=Force Model, 2=Default/Est)
// ===================================================================================
SPose getPose_C(int global_mode, int cmd_mode)
{
	SPose p;

	// --- 1. ПРИОРИТЕТ: СПЕЦ-МАНЕВРЫ (С ОФФСЕТОМ) ---
	// Если G-код требует точного локального маневра (L) или поворота (G1),
	// мы используем "Бесшовный стык" (Model/Odom + Offset).
	// Это перекрывает глобальную настройку, чтобы координаты не скакали.

	if (cmd_mode == 0) // Требуется ODOM + Offset (для L)
	{
		p.x = msg_PoseRotation.x.odom + g_transition_offset.x;
		p.y = msg_PoseRotation.y.odom + g_transition_offset.y;

		double th_sum = msg_PoseRotation.th.odom + g_transition_offset.th;
		while (th_sum > M_PI)
			th_sum -= 2 * M_PI;
		while (th_sum <= -M_PI)
			th_sum += 2 * M_PI;
	...
	if (cmd_mode == 1) // Требуется MODEL + Offset (для Поворотов)
	{
		p.x = msg_PoseRotation.x.model + g_transition_offset.x;
		p.y = msg_PoseRotation.y.model + g_transition_offset.y;
	...
	// --- 2. ОБЫЧНЫЙ РЕЖИМ (ПО ГЛОБАЛЬНОЙ НАСТРОЙКЕ) ---
	// Если команда обычная (cmd_mode == 2), мы возвращаем то, что выбрал пользователь.
	// Здесь нет оффсетов, это "сырые" данные выбранной системы.
	switch (global_mode)
	{
	case 0: // Хочу ездить по Одометрии
		p.x = msg_PoseRotation.x.odom;
		p.y = msg_PoseRotation.y.odom;
		p.th = msg_PoseRotation.th.odom;
		break;

	case 1: // Хочу ездить по Модели
		p.x = msg_PoseRotation.x.model;
		p.y = msg_PoseRotation.y.model;
		p.th = msg_PoseRotation.th.model;
		break;

	case 2: // Хочу ездить по Лидару (Est) - Стандарт
		p.x = msg_PoseRotation.x.est;
		p.y = msg_PoseRotation.y.est;
		p.th = msg_PoseRotation.th.est;
		break;
