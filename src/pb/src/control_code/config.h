...
// ===================================================================================
// SCommand: расширенная структура с координатами до/после (обновленная для 3 режимов)
// ===================================================================================
struct SCommand
{
    int mode;

    // --- НОВОЕ ПОЛЕ: ИСТОЧНИК УПРАВЛЕНИЯ ---
    // 0 = ODOM  (Только энкодеры - для микро-маневров L)
    // 1 = MODEL (Энкодеры + Гироскоп - для поворотов)
    // 2 = EST   (Лидар + Фильтр - для глобальной езды)
    int control_source; 
    ...
    SCommand()
        : mode(0), 
          control_source(2), // По умолчанию едем по EST (самый безопасный вариант)
          duration(0.0f), velL(0.0f), velR(0.0f),
