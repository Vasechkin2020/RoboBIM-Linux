...
double lidar_bias = 0;               // Переменная для bias лидара.Это наколько он не ровно стоит на роботе по отношению к направлению движения.
...
struct SPoseLidar // Варианты расчетов координат лидара
{
    ...
    int countFindPillar;    // Колличество найденных столбов
    int countComparePillar; // Колличество сопоставленных столбов
    int countDistDirect;    // Колличество сопоставленных столюов при расчете по растоянию
    int countCrossCircle;   // Количество пересечений окружностей при расчете по углам
    float quality_ComparePillar; // Качество распознавания столбов (сумма всех гипотенуз / количество найденных столбов)
