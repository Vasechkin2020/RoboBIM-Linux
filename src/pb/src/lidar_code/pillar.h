...
#define PILLAR_RADIUS_ 0.1575  // Радиус столба 315 мм
#define PEREHOD 0.1            // Разрыв между столбом и обьектом за ним
...
public:
    ...
    SPose getLocationmodeDist(SDistDirect *distDirect, SPose pose_, float accuracy_);                         // Метод возврщает положение центра лидара усредненного по всем столбам и расчетам по меоду расчета по "растояние до столбов по лидару"
    SPose getLocationmodeAngle(SDistDirect *distDirect, SPose pose_, float accuracy_);                        // Метод возврщает положение центра лидара усредненного по всем столбам и расчетам по методу расчета по "углам межлу столбами по лидару"
private:
    struct SLidar
    {
...
SPose CPillar::getLocationmodeDist(SDistDirect *distDirect, SPose pose_, float accuracy_) // На вход подается последняя полученная/посчитанная позиция лидара
{
    ...
    for (int i = 0; i < countPillar - 1; i++) //        Перебираем столбы, и для каждого пересечение окружностей со следующим
    {
        ...
        {
            c1.x = distDirect[i].x_true; // Формируем окружности
            c1.y = distDirect[i].y_true;
            c1.r = distDirect[i].distance + PILLAR_RADIUS_;
            c2.x = distDirect[j].x_true;
            c2.y = distDirect[j].y_true;
            c2.r = distDirect[j].distance + PILLAR_RADIUS_;
            int rez = getCrossing(c1, c2, pose_, point, accuracy_); // Считаем пересечение окружнойстей и в итоге получаем текущее положение по 2 окружностям отбираем те точки котрые не дальше 0,1 метра
...
SPose CPillar::getLocationmodeAngle(SDistDirect *distDirect, SPose pose_, float accuracy_) // На вход подается последняя полученная/посчитанная позиция лидара
{
    ...
    for (int i = 0; i < countPillar - 1; i++) //        Перебираем столбы, и для каждой пары формируем окружность
    {
        for (int j = i + 1; j < countPillar; j++)
        {
            p1.x = distDirect[i].x_true; // Формируем пары столбов, получаем угол между ними и если он в пределах от 30 до 150 градусов то считаем окружность которую потом переберем
            p1.y = distDirect[i].y_true;
            p2.x = distDirect[j].x_true;
            p2.y = distDirect[j].y_true;
            a1 = distDirect[j].direction - distDirect[i].direction;
            ...
            if (a1 > 30 && a1 < 150)                // Проверка угла. Если вне диапазона то результаты не точные
            {
                ...
                count_circleAnglePillar++;
                ROS_INFO("    getCircle => c1 x= %.3f y= %.3f c1 = %.3f | c2 x= %.3f y= %.3f c2 = %.3f ", c.c1.x, c.c1.y, c.c1.r, c.c2.x, c.c2.y, c.c2.r);
            }
        }
    }
    ...
    for (int i = 0; i < count_circleAnglePillar - 1; i++) //        Перебираем окружности, и для каждого пересечение окружностей со следующим
    {
        for (int j = i + 1; j < count_circleAnglePillar; j++)
        {
            ...
            int rez = getCrossing(c1, c2, pose_, point, accuracy_); // Считаем пересечение окружнойстей и в итоге получаем текущее положение по 2 окружностям
...
int CPillar::getCrossing(SCircle c1_, SCircle c2_, SPose pose_, SPoint &pointX_, float len_) // https://algolist.manual.ru/maths/geom/intersect/circlecircle2d.php https://planetcalc.ru/8098/ https://paulbourke.net/geometry/circlesphere/
{
    ...
    {
        // ROS_WARN("Circles do not fit the conditions !!! d= %.3f  c1 x= %.3f y= %.3f c1 = %.3f | c2 x= %.3f y= %.3f c2 = %.3f ", d, c1_.x, c1_.y, c1_.r, c2_.x, c2_.y, c2_.r);
    ...
    if (vl3 > len_ || isnan(vl3)) // Если меньший вектор больше минимального заданного то пересечения не подходят или
    {
        ...
        pointX_.y = 0;
        // ROS_INFO("Error getCrossing. vl3 > Len = %.3f m vl3 = %.3f", len_, vl3);
        return ret;
    }
    ret = 1; // Все хорошо
    // ROS_INFO("OK getCrossing. vl3 > Len = %.3f m vl3 = %.3f | pointX_ x= %.3f y= %.3f \n", len_, vl3, pointX_.x, pointX_.y);
    return ret;
}
...
void CPillar::searchPillars(const sensor_msgs::LaserScan::ConstPtr &scan, SPose &poseLidarMode_)
{
    ...
    for (int i = 0; i < count; i++) // Пробегаем по всему массиву, копируем в свой и считаем сколько хороших данных пришло и для дальнейшего преобразования
    {
        ...
        degree = degree + lidar_bias;                                        // учитываем смещение от неточной установки
        degree = normalizeAngle180(degree);                           // Нормализуем угол если перескачили 360
        // if(degree < -150)
        //     printf("1 = %+8.3f 2 = %+8.3f \n", degree, degree2);
    ...
    {
        k--;
        lidarData[i].degree = 180 - lidarDataSrc[k].degree; // Конец копируем в начало и поворачиваем на 180
...
void CPillar::poiskPillar(int a_, int b_, SLidar *lidarData, SPose &poseLidarMode_)
{
    ...
    if (width > 0.2 && width < 0.4) // Если ширина похожа на наш столб
    {
        ...
        pillarLidar[countPillarLidar].x_lidarXY = cos(DEG2RAD(-pillarLidar[countPillarLidar].azimuth)) * (pillarLidar[countPillarLidar].dist_min + PILLAR_RADIUS_); // Находим координаты по формулам. К минимальному растоянию прибавляем радиус столба
        pillarLidar[countPillarLidar].y_lidarXY = sin(DEG2RAD(-pillarLidar[countPillarLidar].azimuth)) * (pillarLidar[countPillarLidar].dist_min + PILLAR_RADIUS_); // Находим координаты по формулам. К минимальному растоянию прибавляем радиус столба
