...
const double BAD_ANGLE_LOW_DEG = 25.0; // Нижняя граница "плохого" угла (для веса)
const double BAD_ANGLE_HIGH_DEG = 155.0; // Верхняя граница "плохого" угла (для веса)
...
TrilaterationSolver::TrilaterationSolver(SPoint prev) : 
    A_prev(prev) 
{
...
void TrilaterationSolver::add_filtered_circle_from_angle(SPoint P1, SPoint P2, double angle_deg) 
{
    ...
    double dynamic_weight_norm = 1e-9; // Итоговый нормированный вес (по умолчанию почти 0)
    double W_angle = 1e-9; // Вес угла
    double W_distance = 1e-9; // Вес дальности
    ...
    printf("dynamic_weight_norm = %.6f (W_angle = %.6f, W_distance = %.6f)\n", // Output calculated weight
           dynamic_weight_norm, W_angle, W_distance); 
    // --- КОНЕЦ БЛОКА ---
...
bool TrilaterationSolver::perform_wls_pass(SPoint& result_A, double& result_rms, int& used_count, bool print_residuals)
{
    ...
    {
        if (print_residuals) printf("Error: No active base circle found for WLS.\n"); // Output error
        return false; 
    }
    ...
    for (int i = 0; i < N; ++i) 
    {
        if (i == base_index) continue; // Пропускаем базовое уравнение
        if (!this->all_circles[i].is_active) continue; // Пропускаем отброшенные
        ...
        double WAi = Wi * Ai; double WBi = Wi * Bi; double WCi = Wi * Ci; 

        // Накопление элементов M^T*W*M
        M_xx += Ai * WAi; M_xy += Bi * WAi; M_yy += Bi * WBi; 
        // Накопление элементов M^T*W*C
        V_x += Ai * WCi; V_y += Bi * WCi; 
    } 
    
    if (active_equations < 2) // Должно быть хотя бы 2 уравнения для X и Y
    {
        if (print_residuals) printf("Error: Not enough active equations (%d) for WLS (min 2 required).\n", active_equations); // Output error
        return false;
    }
    ...
    if (fabs(det) < 1e-9) 
    {
        if (print_residuals) printf("Error: Determinant (%.2e) is near zero. Solution is unstable.\n", det); // Output error
        return false; 
    } 
    ...
    for (int i = 0; i < N; ++i) 
    {
        ...
        if (!current.is_active) continue; // Только активные
...
SPoint_Q TrilaterationSolver::find_A_by_mnk_robust()
{
    ...
    printf("\n--- SOLVER: ROBUST (Two-Pass) (Total N=%d) Aprev x= %6.3f y= %6.3f ---\n", N, A_prev.x, A_prev.y); // Output solver start
    ...
    printf("Pass 1 Found A: (%.3f, %.3f), Initial RMS: %.3f m\n", A_temp.x, A_temp.y, rms_temp); // Output pass 1 result

    // --- АНАЛИЗ ОСТАТКОВ И ОТБРАКОВКА (на основе A_temp) ---
    int rejected_count = 0; // Счетчик отброшенных
    
    printf("\n--- OUTLIER REJECTION ANALYSIS (Threshold: %.3f m) ---\n", OUTLIER_REJECTION_THRESHOLD_METERS); // Output rejection analysis
    ...
    printf("Total rejected measurements: %d\n", rejected_count); // Output rejected count

    // --- ПРОХОД 2: Финальный расчет ---
    SPoint A_final = A_temp; // Результат для финализации
    ...
    if (rejected_count > 0) // Если были отбракованные
    {
        ...
        {
             printf("Pass 2 Found A (Final): (%.3f, %.3f)\n", A_final.x, A_final.y); // Output pass 2 result
...
double TrilaterationSolver::get_lidar_orientation(
    const SPoint A_found,                          // Найденная позиция
    const std::vector<SPoint>& beacons,             // Координаты маяков
    const std::vector<double>& lidar_angles_deg     // Углы до маяков
)
{
    ...
    double sum_sin = 0.0; // Сумма синусов для LSQ
    double sum_cos = 0.0; // Сумма косинусов для LSQ
    int N = beacons.size(); // Количество маяков
    
    for (int i = 0; i < N; ++i) 
    {
        double alpha_AM_deg = get_azimuth_deg(A_found, beacons[i]); // Азимут от А до Маяка (M)
        double theta_lidar_deg = lidar_angles_deg[i]; // Угол, измеренный лидаром
        
        // Сырой угол ориентации: Psi = Alpha - Theta
        double psi_raw_deg = alpha_AM_deg - theta_lidar_deg; 
        double psi_norm_deg = normalize_angle_deg(psi_raw_deg); // Нормализованный угол (-180, 180)
        double psi_rad = DEG2RAD(psi_norm_deg); // В радианах
    ...
    double orientation_rad = atan2(sum_sin, sum_cos); // Финальный LSQ угол в радианах
    double orientation_deg = RAD2DEG(orientation_rad); // В градусах
