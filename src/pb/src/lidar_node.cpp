...
int main(int argc, char **argv) // Главная функция программы
{
    ...
    ROS_FATAL("\n");
    logi.log("***  lidar_node *** ver. 1.01 *** printBIM.ru 22/11/25 *** 2025 ***\n");
    logi.log("--------------------------------------------------------\n");
    ...
    SPose PoseCalc; // Поза относительно кторой все считаем
    SPose startPose;
    ...
    logi.log_w("+++ Waiting for the /scan topic to become active...\n");                                       // БЛОКИРУЮЩЕЕ ОЖИДАНИЕ ЗАПУСКА ДРУГОЙ НОДЫ // Ожидаем, пока топик сигнала запуска не станет активным
    ros::topic::waitForMessage<sensor_msgs::LaserScan>("/scan", ros::Duration(30));                            // Блокирующий вызов: ждет первое сообщение из топика "/scan" с таймаутом 30 секунд. Замените String на нужный тип сообщения.
    logi.log("    Topic scan received. Starting.\n");                                                          // Топик получен. Начинаем фазу калибровки.
    logi.log("========================================================================================== \n"); // Топик получен. Начинаем фазу калибровки.
    ...
    while (ros::ok() && keep_running) // Пока ROS работает и не нажат Ctrl+C
    {
        ...
        {
            PoseCalc.x = msg_Pose.x.est; // Записываем из переменной топика в местную переменную
            PoseCalc.y = msg_Pose.y.est;
            PoseCalc.th = msg_Pose.th.est;
        ...
        if (flag_msgLidar) // Если пришло сообщение в топик от лидара и мы уже разобрали данные по координатам машинки, а значит можем грубо посчитать где стоят столбы.  И знаем где истинные столбы
        {
            ...
            g_poseLidar.modeAngle = pillar.getLocationmodeAngle(distDirect, PoseCalc, 0.16); // Считаем текущие координаты по столбам На вход старая позиция лидара, на выходе новая позиция лидара
            g_poseLidar.modeDist = pillar.getLocationmodeDist(distDirect, PoseCalc, 0.16);   // Считаем текущие координаты по столбам На вход старая позиция лидара, на выходе новая позиция лидара
            ...
            try
            {
                ...
                SPoint pointCalc;
                pointCalc.x = PoseCalc.x;
                pointCalc.y = PoseCalc.y;
                solver.set_A_prev(pointCalc);            // ТОчка опорная для  расчета
            ...
            // Тут примитивным образом выбираем вариант от которого будем в следу.щий раз считать.
            if (g_poseLidar.quality_mknFused < 10) // Если соответствует качество по методу mnk
            {
                PoseCalc = g_poseLidar.mnkFused; // Позиция относительно которой ищем и распознаем столбы. Это или что посчитали по измерения в прошлый раз или уточненная позиция полученная est
            ...
            else
            {
                ...
                {
                    PoseCalc = g_poseLidar.modeDist; // Позиция относительно которой ищем и распознаем столбы. Это или что посчитали по измерения в прошлый раз или уточненная позиция полученная est
                    logi.log_w("=== 'PoseCalc = modeDist' \n");
                }
                else // В коайнем случае
                {
                    PoseCalc = g_poseLidar.modeClaster; // Позиция относительно которой ищем и распознаем столбы. Это или что посчитали по измерения в прошлый раз или уточненная позиция полученная est
