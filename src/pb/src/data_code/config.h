...
SpiDriver spi_drv; // Создаем глобальный объект драйвера
...
#define PIN_SPI_DRIVER 13 // Пин на котором сидит чипселекс от SPI к Modul на плате это 13
#define PIN_SPI_MODUL 12  // Пин на котором сидит чипселекс от SPI к Modul на плате это 12
#define PIN_SPI_PRINT 5  // Пин на котором сидит чипселекс от SPI к Modul на плате это 5
#define PIN_LED_BLUE 26 // Мигает в цикле что работает
//---------------------------------------------------------------------------------------
#define SPI_CHANNAL_0 0   // Какой из двух каналов инициализируем
#define SPI_CHANNAL_1 1   // Какой из двух каналов инициализируем
#define SPI_SPEED 8000000 // Скорость работы шины SPI
//---------------------------------------------------------------------------------------
...
// Вспомогательная функция для хранения единой точки старта
inline ros::Time& get_global_start_time() 
{
    static ros::Time start_time(0); // Инициализируется один раз при первом вызове
    return start_time;
}

inline uint64_t millis() 
{
    // Проверка: запущен ли ROS (иначе Time::now упадет или вернет 0)
    if (!ros::Time::isValid()) return 0;

    ros::Time& start = get_global_start_time();

    if (start.isZero()) {
        start = ros::Time::now();
    }

    // Защита от отрицательного времени (если часы перевели назад или баг симуляции)
    ros::Time now = ros::Time::now();
    if (now < start) return 0; 

    return (uint64_t)((now - start).toNSec() / 1000000);
}
...
inline uint64_t micros() 
{
    ...
    return (uint64_t)((now - start).toNSec() / 1000);
}
