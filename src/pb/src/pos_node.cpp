...
{
    ROS_INFO("%s -------------------------------------------------------------", NN);
    ROS_WARN("%s        Main Module PrintBIM ROS 1.0 Raspberry Pi 4B  ver 1.4 ", NN);
    ...
    while (ros::ok())
    {
        ...
        if (flag_startPose && flag_dataPillar) //  Основной алгоритм только после того как загрузили начальную позицию и положение столбов
        {
            ...
            if (flag_msgSpeed) // Флаг что пришло сообщение от ноды Data по Speed. Это будет MODE_0
            {
                ...
                calcNewOdom(odomMode11, g_linAngVel.wheel); // На основе линейных скоростей считаем новую позицию и угол для одометрии 100 Герц считаем и потом 10 Герц правим
                calcNewOdom(odomMode12, g_linAngVel.wheel); // На основе линейных скоростей считаем новую позицию и угол для одометрии 100 Герц считаем и потом 10 Герц правим
                calcNewOdom(odomMode13, g_linAngVel.wheel); // На основе линейных скоростей считаем новую позицию и угол для одометрии 100 Герц считаем и потом 10 Герц правим
