...
#define RATE_OUTPUT 0.01 // Частота выводв ROS_INFO
...
// #define DISTANCE_WHEELS 0.33 // Растояние между колесами робота. подобрал экспериментально Влияет на правильность круга
...
uint32_t g_readings_count = 0; // Считаем сколько раз получили данные от data_node. Нужно вначале для калибровки акселерометра и гирскопа

//--------------------------------- ПОДПИСКА НА ТОПИКИ -------------------------------------------------
pb_msgs::Struct_PoseLidar msg_Measurement; // Перемеенная в которую сохраняем данные лидара из сообщения
pb_msgs::Struct_PoseScan msg_Scan; // Перемеенная в которую сохраняем данные лидара из сообщения
...
bool flag_msgMeasurement = false; // Флаг что пришло сообщение в топик и можно его парсить
bool flag_msgScan = false; // Флаг что пришло сообщение в топик и можно его парсить
...
SXyz biasAccel = {0, 0, 0}; // Поправочные коэффициенты
SXyz biasGyro = {0, 0, 0};  // Поправочные коэффициенты
...
struct STwistDt
{
    float vx = 0;  // Линейная скорость движения робота по оси X метры в секунду
    float vy = 0;  // Линейная скорость движения робота по оси Y
    float vth = 0; // Угловая скорость вращения робота в радианах
    double dt = 0.0;
};
...
struct SLinAngVel
{
    ...
    STwistDt imu;   // Скорости полученные по IMU
    STwistDt model; // Обьединенные комплементратный способом Скорости и интервал
};
...
struct SPoseRotation
{
    ...
    SPose imu;   // Расчет чисто по IMU
    SPose model; // Комплементация 
    SPose meas;  // Измерения 
    SPose est;  // ИТОГОВАЯ ПОЗИЦИЯ ИСПОЛЬЗУЕМАЯ ДАЛЕЕ КАК ОСНОВНАЯ
};

struct SPoseBase // Варианты расчетов для точки где установлен лидар это Base система координат
{
    SPose model; // Позиция ИТОГОВАЯ как результат ВСХ ИЗМЕРНИЙ И ВСЕХ РАСЧЕТОВ. Это основа для управления
    SPose meas; // Позиция по всем возмодным ИЗМЕРЕНИЯМ. скомплементированная в ноде расчета измерений
    SPose est; // Позиция ИТОГОВАЯ как результат ВСХ ИЗМЕРНИЙ И ВСЕХ РАСЧЕТОВ. Это основа для управления
