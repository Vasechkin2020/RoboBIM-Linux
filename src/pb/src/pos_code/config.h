...
struct SPoseRotation
{
    ...
    SPose fused; // Комплементация ИТОГОВАЯ ПОЗИЦИЯ ИСПОЛЬЗУЕМАЯ ДАЛЕЕ КАК ОСНОВНАЯ calculated
};
...
struct SPoseBase // Варианты расчетов для точки где установлен лидар это Base система координат
{
    ...
    SPose measurement; // Позиция по всем возмодным ИЗМЕРЕНИЯМ. скомплементированная в ноде расчета измерений
    SPose calculated;  // Позиция по всем возможным РАСЧЕТАМ (одометрия + IMU)
    SPose odom;        // для сравнительной отладки поза только по одометрии
    SPose imu;         // для сравнительной отладки поза только по IMU
