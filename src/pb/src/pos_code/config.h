...
struct SPoseRotation
{
    ...
    SPose fused; // Комплементация 
    SPose meas;  // Измерения 
    SPose main;  // ИТОГОВАЯ ПОЗИЦИЯ ИСПОЛЬЗУЕМАЯ ДАЛЕЕ КАК ОСНОВНАЯ main
};

struct SPoseBase // Варианты расчетов для точки где установлен лидар это Base система координат
{
    SPose main; // Позиция ИТОГОВАЯ как результат ВСХ ИЗМЕРНИЙ И ВСЕХ РАСЧЕТОВ. Это основа для управления
