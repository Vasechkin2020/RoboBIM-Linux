...
public:
    ...
    std::vector<Circle> detectByGap(const std::vector<Point2D> &points)
    {
        ...
        for (size_t i = 1; i < sorted_points.size(); i++)
        {
            ...
            if (dist > GAP_THRESHOLD)
            {
                ...
                { // Минимум 5 точек для аппроксимации
    ...
    std::vector<Circle> weightedFusion(const std::vector<Circle> &candidates)
    {
        ...
        {
            if (cluster.size() >= 2)
            { // Объединяем только если есть несколько кандидатов
                Circle fused;
    ...
    double calculateCandidateWeight(const Circle &circle, int points_count)
    {
        ...
        case 1:
            weight_method = 0.9;
            break; // Метод разрывов
        case 2:
            weight_method = 1.0;
            break; // Кластеризация
        case 3:
            weight_method = 0.8;
            break; // Локальные минимумы
        }
