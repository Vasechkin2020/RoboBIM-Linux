/*
 * File: gen.cpp
 * Version: 3.9.1 (13-12-2025)
 * Changes:
 * - IMPROVED: Finalize logic.
 * - Logic: Regardless of whether parking is defined, the robot now always moves its center 
 *   to the exact end coordinates of the last line (undoing the marker offset) before finishing or parking.
 */
...
private:
    ...
    // Храним координаты конца последней нарисованной линии для корректного выхода
...
public:
    ...
    : trans(t), last_endpoint_x(0), last_endpoint_y(0), has_drawn_anything(false)
    {
        fp = fopen(filename, "w");
        if (fp) {
            fprintf(fp, "; === GENERATED G-CODE (v3.9.1) ===\n"); 
    ...
    void finalize(bool use_parking, double park_x, double park_y) {
        ...
        // ШАГ 1: Если мы что-то рисовали, нужно "дожать" смещение, 
        // чтобы центр робота встал ровно в конец последней линии.
        if (has_drawn_anything) {
            fprintf(fp, "G2 X%.3f Y%.3f F%.2f ; Align Center to Last Point (Undo Offset)\n", 
                    last_endpoint_x, last_endpoint_y, SPEED_MOVE);
            fprintf(fp, "G4 P1000\n");
        }

        // ШАГ 2: Едем на парковку, если она задана
        if (use_parking && has_drawn_anything) {
            // Вычисляем поворот на парковку от ТЕКУЩЕЙ позиции (которая теперь равна last_endpoint)
        ...
        // ШАГ 3: Финальный поворот в ноль (где бы мы ни были: в конце линии или на парковке)
...
int main() {
    ...
    std::vector<std::string> body_lines_raw; // Строки координат (включая потенциальную парковку)
    ...
    while (std::getline(infile, line)) {
        raw_input_lines.push_back(line); // Для бэкапа
        ...
        if (line_count == 0) {
            ...
            // Сохраняем все остальные строки данных
    ...
    // 2. Анализ строк данных: Разделение на линии рисования и точку парковки
    ...
    for (size_t i = 0; i < body_lines_raw.size(); i++) {
        ...
        // Очистка от комментариев для парсинга
        ...
        if (i == body_lines_raw.size() - 1) {
            if (found == 2) {
                ...
                park_x = val[0];
                park_y = val[1];
                applyTransform(park_x, park_y, trans); // Не забываем трансформировать!
                printf("Parking Point Detected: CAD(%.2f, %.2f) -> ROBOT(%.3f, %.3f)\n", val[0], val[1], park_x, park_y);
                continue; // Не добавляем в список линий
            }
        }
        ...
        if (found == 4) {
            ...
            if (found != 2) // Если 2 найдено не в последней строке, это ошибка
                printf("Warning: Skipped invalid line: %s\n", current_line.c_str());
        }
    }
    ...
    // 4. Завершение (с парковкой, если она была найдена)
